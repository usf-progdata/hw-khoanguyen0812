...
m_plot <- m_long_trim %>% 
  ...
  geom_smooth(
    method = "lm", se = FALSE) +
  labs(
    title = "Marriage Trend Amongst Different Age Group Over Time for Men.",
    x = "year",
    y = "Marriage Rate",
    color = "Age Range"
  ) +
...
> As observed, the divorce trend is significantly different from the marriage trend with the difference being the rate is much higher in divorce than in marriage. Furthermore, the trend is growing even more starting from the early 2000s.
...
> I combined both of them together to make the graph.
...
```{r}
Rainfall_graph_for_marriage_and_divorce <- Joined_data_for_M_D_trim %>% 
  ggplot +
  aes(
    x = BAp_3544_Marriage,
    y = BAp_3544_Divorce,
    fill = BAp_3544_Divorce,
    color = BAp_3544_Divorce
  ) +
  geom_jitter(
    alpha = .5,
    height = .2,
