...
dat_bfi %>% 
  mutate(gender = recode(gender, "1" = "Male", "2" = "Female",.default = "no response")) %>% 
  select(.id, gender, education)
...
dat_bfi %>% 
  mutate(education = recode(
    education, "1" = "Some HS", 
                            ...
                            "4"= "College", 
                            "5"= "Graduate Degree")) %>% 
  mutate(education = replace_na(education, replace = "(No Response)")) %>% 
  mutate(gender = recode(gender, "1" = "Male", "2" = "Female",.default = "no response")) %>%
  select(.id, gender, education)
...
dat_bfi %>% 
  mutate(hs_grad = recode(education, "1" = "no", .default = "yes", .missing = "No Response")) %>% 
  select (.id, hs_grad)
