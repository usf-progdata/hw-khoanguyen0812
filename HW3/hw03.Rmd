...
```{r}
green_data %>% 
mutate(student = recode(student, "1" = "Not_Student", "2" = "Student", .missing = "No_Response")) %>% 
  select(id, student)
```
...
```{r}
green_data %>% 
  mutate(student = recode(student, "1" = "Not_Student", "2" = "Student", .default  = "No_Response")) %>% 
  rowwise() %>%
